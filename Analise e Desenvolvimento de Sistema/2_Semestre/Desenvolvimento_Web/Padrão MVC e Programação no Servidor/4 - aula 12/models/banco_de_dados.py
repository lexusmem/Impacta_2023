...
class BancoDeDados:
    ...
    def inserir_aluno(self, obj_aluno):
        ...
        connection = self.__engine.connect()
        t = sqlalchemy.text(
            "INSERT INTO Aluno VALUES (:usuario, :senha, :nome, :curso, :data_inicio, :media)")
        ...
        hash_da_senha = hashlib.sha512(
            senha_original.encode('UTF-8')).hexdigest()

        connection.execute(t, usuario=obj_aluno.usuario, senha=hash_da_senha, nome=obj_aluno.nome,
                           curso=obj_aluno.curso, data_inicio=obj_aluno.data_inicio, media=obj_aluno.media)
        connection.close()
    ...
    def listar_alunos(self):
        ...
        resultado = connection.execute(
            "SELECT usuario, senha, nome, curso, data_inicio, media FROM Aluno ORDER BY usuario")
        for linha in resultado:
            # linha é uma tupla com todos esses campos, portanto podemos "descompacta-la" nessas 6 variáveis
            usuario, senha, nome, curso, data_inicio, media = linha
            # criamos um objeto aluno com as variáveis
            alu = Aluno(usuario, senha, nome, curso, data_inicio, media)
            # adicionamos o objeto à lista que será retornada pelo método
            alunos.append(alu)
        connection.close()
        return alunos
    ...
    def obter_aluno(self, usuario):
        ...
        connection = self.__engine.connect()
        t = sqlalchemy.text(
            "SELECT usuario, senha, nome, curso, data_inicio, media FROM Aluno WHERE usuario = :user")
        resultado = connection.execute(t, user=usuario)
        # obtem o primeiro resultado (mas a nossa consulta sempre retornará no máximo 1 linha do BD)
...
if __name__ == '__main__':
    ...
    if opcao == 1:
        bd.criar_tabelas()
        bd.inserir_aluno(Aluno('rafael', '1234', 'Rafael Will',
                         'Ciência da Computação', '01/02/2022', 7.5))
        bd.inserir_aluno(Aluno('maria', '4321', 'Maria dos Santos',
                         'Análise e Desenvolvimento de Sistemas', '11/08/2022', 8.6))
        bd.inserir_aluno(Aluno('jose', '9876', 'José Silva',
                         'Sistemas de Informação', '31/05/2022', 6.9))
        bd.inserir_aluno(Aluno('ana', '5678', 'Ana Beatriz',
                         'Engenharia da Computação', '05/10/2022', 9.2))
    else:
